...
def YOLOv4_tiny(input_layer, NUM_CLASS):
    ...
    numMasks = 2
    conv_lbbox = common.convolutional(conv_lobj_branch, (1, 1, 512, numMasks * (NUM_CLASS + 5)), activate=False, bn=False)

    conv = common.convolutional(conv, (1, 1, 256, 128))
    conv = common.upsample(conv)
    conv = tf.concat([conv, route_1], axis=-1)

    conv_mobj_branch = common.convolutional(conv, (3, 3, 384, 256))
    conv_mbbox = common.convolutional(conv_mobj_branch, (1, 1, 256, numMasks * (NUM_CLASS + 5)), activate=False, bn=False)
...
def decode(conv_output, width, height, NUM_CLASS, STRIDES, ANCHORS, i, XYSCALE=[1,1,1], FRAMEWORK='tf'):
    if FRAMEWORK == 'trt':
        return decode_trt(conv_output, width, NUM_CLASS, STRIDES, ANCHORS, i=i, XYSCALE=XYSCALE)
    elif FRAMEWORK == 'tflite':
        return decode_tflite(conv_output, width, height, NUM_CLASS, STRIDES, ANCHORS, i=i, XYSCALE=XYSCALE)
    else:
        return decode_tf(conv_output, width, height, NUM_CLASS, STRIDES, ANCHORS, i=i, XYSCALE=XYSCALE)
...
def decode_tf(conv_output, width, height, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1, 1, 1]):    
    batch_size = tf.shape(conv_output)[0]    
    print("decode_tf", height, width)    
    conv_output = tf.reshape(conv_output,
                             (batch_size, height, width, 1, 5 + NUM_CLASS))
    print(conv_output)
    conv_raw_dxdy, conv_raw_dwdh, conv_raw_conf, conv_raw_prob = tf.split(conv_output, (2, 2, 1, NUM_CLASS),
                                                                          axis=-1)

    xy_grid = tf.meshgrid(tf.range(width), tf.range(height))
    xy_grid = tf.expand_dims(tf.stack(xy_grid, axis=-1), axis=2)  # [gx, gy, 1, 2]    
    xy_grid = tf.tile(tf.expand_dims(xy_grid, axis=0), [batch_size, 1, 1, ANCHORS[i].shape[0], 1])

    xy_grid = tf.cast(xy_grid, tf.float32)
    print("XYSCALE[i] ", XYSCALE[i])
    print("XYSCALE ", XYSCALE)
    
    pred_xy = ((tf.sigmoid(conv_raw_dxdy) * XYSCALE[i]) - 0.5 * (XYSCALE[i] - 1) + xy_grid) * \
              STRIDES[i]
    print("ANCHORS[i]", ANCHORS[i])
    pred_wh = (tf.exp(conv_raw_dwdh) * ANCHORS[i])
    pred_xywh = tf.concat([pred_xy, pred_wh], axis=-1)
...
def decode_tflite(conv_output, width, height, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1,1,1]):
    conv_raw_dxdy_0, conv_raw_dwdh_0, conv_raw_score_0,\
    conv_raw_dxdy_1, conv_raw_dwdh_1, conv_raw_score_1 = tf.split(conv_output, (2, 2, 1+NUM_CLASS, 2, 2, 1+NUM_CLASS), axis=-1)    

    conv_raw_score = [conv_raw_score_0, conv_raw_score_1]
    ...
    conv_raw_dxdy = [conv_raw_dxdy_0, conv_raw_dxdy_1]
